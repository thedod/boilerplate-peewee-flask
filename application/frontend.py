...
from sitepack.nav import nav, ExtendedNavbar, BabelView
from sitepack.db import peewee_flask_utils
from sitepack.babel_by_url import babel_config
...
def frontend_top_nav():
        navbar = ExtendedNavbar(
            View(babel_config('SITE_TITLE'), 'frontend.index'),
            root_class='navbar navbar-inverse navbar-fixed-top',
            items = (
                BabelView(_('Home'), 'frontend.index'),
                BabelView(_('Members'), 'frontend.members'),
            )
        )
        if current_user.is_active:
            navbar.right_items = (
                Text('{}:'.format(current_user.email)),
                View(_('Logout'), 'security.logout'),
                View(_('Change password'), 'security.change_password'),
            )
            if current_user.has_role('editor'):
                navbar.right_items += \
                    (View(_('Site editor'), 'backend.index'),)
            if current_user.has_role('admin'):
                navbar.right_items += \
                    (View(_('User admin'), 'useradmin.index'),)
        else:
            navbar.right_items = ( View(_('Login'), 'security.login'), )

        if g.language=='en':
            navbar.right_items += (
                BabelView(u'עברית', 'frontend.index', babel_language='he'), )
        else:
            navbar.right_items += (
                BabelView('English', 'frontend.index', babel_language='en'), )
        return navbar
